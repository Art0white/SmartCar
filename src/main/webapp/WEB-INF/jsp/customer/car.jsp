...
<%--		<script type="text/javascript" src="http://static.robotwebtools.org/roslibjs/current/roslib.min.js"></script>--%>
				...
				<div class="ui grid">
					...
					<div class="seven wide column">
						<div class="two column row">
							<%--左七上--%>
							<div class="column">
								<div id="carvideo" class="carvideo" style="align-items: center; padding-top: 20px">
									<img src="http://10.100.71.139:5000/stream?topic=/rrbot/camera1/image_raw" alt="" style="width:auto; height:auto; max-width:100%; max-height:100%;">
								</div>
							</div>
							<%--左七下--%>
						...
						<div class="two column row">
							...
							<div class="column">
								<div id="carmapvideo" class="carmapvideo">
									<div id="map" style="width:100%; padding-top: 15px; "></div>
								</div>
							</div>
							<%--右九下--%>
							<div class="column" >
	...
	function getDvsStatus(statusNum) {
		if(statusNum === 0) {
			return "OK";
		} else if(statusNum === 1) {
			return "Error";
	...
	var statucOfCar = "Unknow";
	resp.callService(request, function(result) {
		statucOfCar = getDvsStatus(result.status);
		console.log("机器0的状态: " + getDvsStatus(result.status))
		document.getElementById("speeddata").innerHTML = `<h3>小车速度</h3> <p>` + sd + `</p> <h3>设备0的状态</h3> <p>` + statucOfCar + `</p>`;
	});
	...
	var carWorkingOut = new ROSLIB.Topic ({
		ros : ros,
		name : '/routing',
		messageType : 'std_msgs/String'
	});

	carWorkingOut.subscribe(function(result) {
		swal("前方传来","小车巡检任务完成(*^▽^*)！");
		resp.callService(request, function(result) {
			statucOfCar = getDvsStatus(result.status);
			console.log("机器0的状态: " + getDvsStatus(result.status))
